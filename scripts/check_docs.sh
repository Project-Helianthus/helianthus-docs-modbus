...
check_sunspec_v2_contract() {
  ...
  grep -Fq '| 702 | 50 | DER capacity |' "$document"
	...
	grep -Fq '| 707 | `7 + NCrvSet` | DER Trip LV observed state with repeated curve observations |' "$document"
	grep -Fq '| 708 | `7 + NCrvSet` | DER Trip HV observed state with repeated curve observations |' "$document"
	grep -Fq '| 709 | `7 + NCrvSet` | DER Trip LF observed state with repeated curve observations |' "$document"
  ...
  grep -Fq 'No point in either model creates a write method, send authority, operation admission,' "$document"
	...
	grep -Fq 'Model 707 has data-register length `7 + NCrvSet`.' "$document"
	grep -Fq '`NCrvSet` is at payload-register offset 4 and absolute model word 6.' "$document"
	grep -Fq 'The source-schema length permits a non-sentinel count from 0 through 65528.' "$document"
	grep -Fq 'Each repeated `Crv` observation consumes exactly 1 data register.' "$document"
	grep -Fq 'It exceeds the current 65,536-word isolated offline extent boundary, so it remains raw-only opaque with zero decoded facts pending a separately reviewed aggregate-extent contract.' "$document"
	...
	grep -Fq 'Model 708 has data-register length `7 + NCrvSet`.' "$document"
	grep -Fq 'Model 708 does not infer a relationship to Models 707, 709, or 710.' "$document"
	grep -Fq 'Every Model 708 field, including `Ena`, `AdptCrvReq`, and repeated `Crv.ReadOnly`, is observed state only and is `NO_SEND`.' "$document"
	grep -Fq 'No Model 708 field creates a write method, send authority, operation admission,' "$document"
	grep -Fq 'Model 709 has data-register length `7 + NCrvSet`.' "$document"
	grep -Fq 'Model 709 retains `Hz_SF` and `Tms_SF` as observed scale-factor fields.' "$document"
	grep -Fq 'Model 709 does not infer a relationship to Models 707, 708, or 710.' "$document"
	grep -Fq 'Every Model 709 field, including `Ena`, `AdptCrvReq`, and repeated `Crv.ReadOnly`, is observed state only and is `NO_SEND`.' "$document"
	grep -Fq 'No Model 709 field creates a write method, send authority, operation admission,' "$document"
