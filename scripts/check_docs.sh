...
check_bms_contract() {
  ...
  grep -Fq 'must return the selected unit, revision tuple, and all four exact FC03 slices' "$document"
  grep -Fq 'must preserve every retained word at its native offset' "$document"
...
check_growatt_protocol_ii_identity_projection() {
  ...
  grep -Fq 'The identity projection does not authorize an FC06, FC16, or other write.' "$document"
