...
check_sunspec_v2_contract() {
  ...
  grep -Fq 'Models 701, 702, 703,' "$document"
	grep -Fq '713, 714, 715, 802, 803, and 804' "$document"
  ...
  grep -Fq 'No point in either model creates a write method, send authority, operation admission,' "$document"
	...
	grep -Fq 'Models 805 through 809 remain excluded pending separate family and substructure decisions.' "$document"
  ...
  grep -Fq 'the declared length, Model 714 is a raw-only opaque block.' "$document"
	...
	grep -Fq 'Each repeated string consumes exactly 32 data registers.' "$document"
	...
	grep -Fq 'Each repeated module consumes exactly 16 data registers.' "$document"
