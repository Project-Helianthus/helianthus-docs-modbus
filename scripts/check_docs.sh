...
check_tesla_tedapi_contract() {
  ...
  grep -Fq 'This version defines a qualified WC system-information operation:' "$document"
  grep -Fq '`tesla.hsc.fc100.wc_system_info.v1`.' "$document"
  grep -Fq 'nested request message is exactly `32 02 4a 00`; therefore its FC100 PDU is' "$document"
  grep -Fq 'exactly `04 32 02 4a 00`.' "$document"
  grep -Fq 'optional FC100 intermediate and may occur no more than once.' "$document"
  grep -Fq 'nested envelope contains WC family `6` and one response tag `10`.' "$document"
  grep -Fq 'tag-`10` body is a bounded opaque terminal body.' "$document"
  grep -Fq 'not a WC system-information result and must fail closed' "$document"
  grep -Fq 'general application failure is an FC100 normal response whose nested envelope contains Common family `4` and error tag `1`;' "$document"
