...
check_sunspec_v2_contract() {
  ...
  grep -Fq 'Models 701, 702, 703,' "$document"
	grep -Fq '713, 714, 715, and 802' "$document"
  ...
  grep -Fq 'No point in either model creates a write method, send authority, operation admission,' "$document"
	...
	grep -Fq 'Models 803 through 809 remain excluded pending separate family and substructure decisions.' "$document"
...
check_sunspec_v2_licensing() {
  local document="$1"
	...
	grep -Fq '| `sunspec.der.readonly.v2` | Common 1/66 plus Models 701/153, 702/50, 703/17, 713/7, 714 variable geometry, 715/7, and 802/62 at the exact pinned V2 schema revision | public Apache-2.0 model catalogue and independently stated interoperability facts | offline decoder contract; runtime, vendor, and catalog admission default denied |' "$document"
