...
check_tesla_evse_scope() {
	local document="$1"
	local forbidden='wifi|wi-fi|\bAP\b|pairing|provision|registration|activat(e|ion)|credential|password|auth(entication|orization)?|OCPP|LED|service|debug|eCAN|mailbox|factory[[:space:]-]?reset|perform[[:space:]-]?update|check[[:space:]-]?for[[:space:]-]?update|clear[[:space:]-]?update|reboot|firmware[[:space:]-]?update|AccessControl|ChargeSchedule|SecurityParameter|CountryCode|Neurio'
